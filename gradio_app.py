...
def run_fontdiffuer(source_image, 
    ...
    args.batch_size = min(batch_size, 1)  # Limit batch_size to 1 to reduce memory usage
    args.seed = random.randint(0, 10000)
    pipe = load_font_diffuser_pipeline(args=args)
    if pipe is None:
        return gr.Error("Failed to load pipeline due to memory or configuration issues.")
    try:
        out_image = sampling(
            args=args,
            pipe=pipe,
        ...
        torch.cuda.empty_cache()  # Clear memory (CPU fallback)
        return out_image
    except Exception as e:
        torch.cuda.empty_cache()  # Clear memory on error
        return gr.Error(f"Sampling failed: {str(e)}")
